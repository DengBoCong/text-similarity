...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if self.segment_type == "absolute":
            ...
            token_type_embeddings = self.token_type_embeddings(torch.zeros_like(token_type_ids))
...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if self.segment_type == "relative":
            self.register_parameter("segment_embedding", param=nn.Parameter(torch.zeros(2, self.attention_head_size)))
    ...
    def forward(
        ...
        if self.segment_type == 'relative':
            ...
            token_type_distance = torch.abs(token_type_ids_l - token_type_ids_r)  # (b, l, r)
            token_type_distance = token_type_distance.unsqueeze(1)
            # (b,h,l,2)
            segment_embedding = torch.matmul(query_layer, segment_tensor.T)
            relative_segment_scores = torch.where(token_type_distance > 0, segment_embedding[..., 1:],
                                                  segment_embedding[..., 0:1])
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.use_first_two:
            first_token_tensor = torch.mean(hidden_states[:, 0:2], dim=1)
        else:
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.pooler = BertPooler(config) if add_pooling_layer and not self.config.use_mean_pooling else None
    ...
    def forward(
        ...
        if self.pooler is not None:
            ...
            pooled_output = mean_pooling(sequence_output, attention_mask=attention_mask)
        else:
            pooled_output = None
