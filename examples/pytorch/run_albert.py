...
class Model(nn.Module):
    ...
    def __init__(self, bert_config: BertConfig, batch_size: int):
        ...
        self.albert = ALBERT(config=bert_config, batch_size=batch_size, with_pool=True)
...
def actuator(model_dir: str, execute_type: str) -> NoReturn:
    ...
    if execute_type == "preprocess":
        ...
        weight_dict = load_bert_weights(
            model_file_path=model_file_path, model=model, mapping=albert_variable_mapping(prefix_="albert.")
        )
...
if __name__ == '__main__':
    actuator(model_dir="./data/ch/bert/albert_chinese_small", execute_type="train")
