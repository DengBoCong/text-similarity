...
class CustomPipeline(TextPairPipeline):
    ...
    def _train_step(self, batch_dataset: dict, optimizer: torch.optim.Optimizer, *args, **kwargs) -> dict:
        ...
        loss = self._metrics(labels, outputs)
...
def actuator(execute_type: str, model_type: str, model_dir: str = None) -> NoReturn:
    ...
    if execute_type == "preprocess":
        # 按照原论文中的需要在seq起始位置替换成[Q]或[D]，由于作者没有提供可用预训练模型，暂时直接使用原生BERT预训练模型
        ...
        model = ColBERT(config=bert_config, batch_size=batch_size, bert_model_type=model_type,
                        mask_punctuation=mask_punctuation, tokenizer=tokenizer)
