...
def infer(samples: list,
    ...
    bm25 = BM25(ngram_samples)
    bm25_feature = bm25.weight()

    bm25_feature_np = np.zeros((len(ngram_samples), len(word_index)))
    for index, sample in enumerate(ngram_samples):
        for word, word_tf_idf in zip(sample, bm25_feature[index]):
            bm25_feature_np[index][word_index[word]] = word_tf_idf

    train_data = bm25_feature_np[:-infer_size]
    infer_data = bm25_feature_np[-infer_size:]
