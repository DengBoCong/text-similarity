...
from sim.tensorflow.modeling_deberta import DeBERTa
...
def actuator(model_dir: str, execute_type: str) -> NoReturn:
    ...
    h5_model_path = os.path.join(model_dir, "tf_model.h5")
    dict_path = os.path.join(model_dir, "vocab.json")
    ...
    if execute_type == "preprocess":
        ...
        input_ids = keras.Input(shape=(40,))
        type_ids = keras.Input(shape=(40,))
        deberta = DeBERTa(config=bert_config, batch_size=batch_size, with_pool=True)([input_ids, type_ids])
        model = keras.Model(inputs=[input_ids, type_ids], outputs=deberta.output)
        model.summary()
        exit(0)
