...
def infer(samples: list,
    ...
    """
    ngram_samples, token_counter = [], Counter()
    for sample in samples:
        # 中文词这么处理
        sample = sample.replace(" ", "")
        ngram_sample = []
        for ngram_num in range(ngram_range[0], ngram_range[1]):
            tmp = ["".join(item) for item in ngrams(sample, ngram_num)]
            ngram_sample.extend(tmp)
            token_counter.update(tmp)

        ngram_samples.append(ngram_sample)

    word_index = {}
    for index, (k, v) in enumerate(token_counter.most_common()):
        if v <= 2:
            break
        word_index[k] = index

    tf_idf = TFIdf(ngram_samples)
    tf_idf_feature = tf_idf.weight()

    tf_idf_feature_np = np.zeros((len(ngram_samples), len(word_index)))
    for index, sample in enumerate(ngram_samples):
        for word, word_tf_idf in zip(sample, tf_idf_feature[index]):
            tf_idf_feature_np[index][word_index[word]] = word_tf_idf

    train_data = tf_idf_feature_np[:-infer_size]
    infer_data = tf_idf_feature_np[-infer_size:]
